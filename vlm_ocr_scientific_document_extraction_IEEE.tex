...
\begin{abstract}
Technical laboratory reports contain critical measurement data, yet automated extraction remains challenging due to varied layouts and specialized formatting. Four Vision Language Model architectures were evaluated on 15 reports spanning 1,374 pages. The olmOCR-2-7B model achieved optimal performance with a 100\% success rate and extracted 3,778 data rows, requiring 104 minutes per report on an RTX 6000 GPU. Enhanced prompting eliminated classification errors entirely but increased processing time 7.8-fold, revealing a fundamental accuracy-efficiency tradeoff. Analysis identified 547 distinct table structures across reports and consolidated 2,856 column variants into 147 normalized patterns, producing 16,834 machine learning-ready rows. Results demonstrate that mid-sized, domain-tuned models outperform larger generic architectures for technical document automation.
\end{abstract}
...
Mid-sized VLMs with domain-specific fine-tuning are hypothesized to outperform both smaller generic models and larger general-purpose architectures for technical document extraction. Enhanced quality control prompting is expected to substantially improve accuracy at the cost of longer processing times. Systematic consolidation is predicted to unify heterogeneous extraction outputs into structured datasets despite significant format variations. This work provides empirical evidence for VLM deployment in scientific document automation and establishes methodology for production implementation.
...
\subsection{Representation, Domain Shift and Data Quality in Applied ML}

Performance in pattern recognition depends heavily on representation choices and parameterization. In face recognition, the selection of image transform and its step-size parameters produces substantial accuracy differences \cite{dargham2010}. This principle extends directly to document extraction: the choice of image rendering resolution (DPI), page-type framing and table structure constraints all function as representation decisions that govern model performance.

Domain shift is an equally critical concern in deployment. Training data selection directly determines whether a system generalizes to real-world conditions \cite{dargham2015}. In document extraction, laboratory reports vary across vendors, time periods and layout conventions. Model success therefore depends on exposure to representative layout and quality conditions. Workflow design must explicitly account for this distribution shift across page types and table styles.

Data deficiency, quality constraints and representational heterogeneity are recognized as recurring bottlenecks across applied machine learning domains. In plastic waste detection, for example, dataset scarcity and color-space representation choices are identified as primary performance constraints \cite{tamin2022}. The schema consolidation task in this paper addresses an analogous bottleneck: heterogeneous column naming across historical extractions constitutes a dataset quality problem that must be resolved before downstream machine learning becomes feasible.

\section{Methodology}

\subsection{Dataset Description}

Fifteen technical laboratory reports totaling 1,374 pages were evaluated. Reports originated from multiple vendors with heterogeneous formatting, containing porosity-permeability measurements, capillary pressure curves and electrical properties. Document size ranged from 17 to 421 pages per report (median 55 pages). Content comprised metadata pages (19.2\%), data tables (50.3\%), graphs (23.2\%) and blank pages (5.8\%). Tables showed substantial diversity with 1--17 columns and varying row counts.
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{fig6_workflow.png}}
...
The four architectures revealed stark performance differences (Table~\ref{tab:model_comparison}, Fig.~\ref{fig:model_performance}). The olmOCR-2-7B model completed processing in 36 minutes while extracting all 160 ground truth rows. The Qwen3-VL-4B took 81 minutes but captured only 76 rows due to excessive hallucination. The 30B model exceeded 3 hours before termination, proving impractical despite its theoretical advantages. These results position olmOCR as the clear choice for production deployment.
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=0.8\columnwidth]{fig1_model_performance.png}}
...
Page type distribution across all documents (Fig.~\ref{fig:page_distribution}) confirmed that data tables dominate at 50.3\%, validating the focus on table extraction. GPU memory remained stable at 15.45 GB throughout, enabling concurrent processing on high-capacity hardware.

\begin{figure}[htbp]
\centerline{\includegraphics[width=0.8\columnwidth]{fig2_page_distribution.png}}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=0.8\columnwidth]{fig5_scalability.png}}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{fig3_prompt_comparison.png}}
...
General extraction without classification assumptions revealed remarkable diversity (Table~\ref{tab:general_extraction}). A total of 547 distinct table structures with 1--17 columns and varying semantic categories were identified. This heterogeneity confirms that universal extraction strategies cannot optimize across all table types. Domain-specific approaches targeting known structures achieve superior results.
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=0.8\columnwidth]{fig4_consolidation.png}}
\caption{Progressive column variant reduction.}
\label{fig:consolidation}
\end{figure}

While deterministic text processing handled syntactic variations, semantic variants required domain knowledge. Abbreviations such as ``CPOR'', ``He Porosity'' and ``Phi\_He'' all represent identical measurements but require expert mapping to unify correctly.
...
Identifying 547 distinct table structures across 15 reports demonstrates substantial format diversity stemming from different vendors, evolving standards and sample-specific customization. This validates focusing on use-case-driven extraction rather than universal approaches. As shown for training data selection in vision systems \cite{dargham2015}, the distribution of layout types seen during development directly determines real-world generalization.

Target applications typically need only two or three specific table types per report. Focused workflows achieve superior accuracy with reduced cost compared to general extraction. Production systems should classify table types first and then route to specialized extraction prompts optimized for specific structures.

\subsection{Consolidation Challenges}

Reducing 2,856 column variants to 147 patterns reveals substantial naming inconsistency. Beyond syntactic variations, semantic variants require domain knowledge. Abbreviation expansion, unit standardization and property synonyms all need expert knowledge or comprehensive ontologies. This mirrors the representation challenge noted in applied ML: just as color-space choices constrain plastic waste detection performance \cite{tamin2022}, column naming heterogeneity constrains downstream ML readiness.
...
Future research should explore hybrid workflows combining rapid general extraction with targeted high-accuracy extraction for critical fields. Active learning leveraging human corrections can fine-tune models for domain-specific layouts. Multimodal validation incorporating graph analysis can cross-validate tabular data against plots. Distributed processing frameworks enable production-scale throughput. Representation sensitivity analysis, analogous to step-size parameter studies in classical vision \cite{dargham2010}, should be conducted for DPI and prompt structure choices.

\section{Conclusion}

This study demonstrates the technical viability of VLM-based automation for scientific document extraction while revealing critical tradeoffs. The olmOCR-2-7B architecture achieved 100\% processing success across 15 heterogeneous reports, extracting 3,778 data rows at an average of 104 minutes per document. Quality control prompting eliminated classification errors entirely at 7.8$\times$ computational cost, quantifying the fundamental accuracy-efficiency tension.

A total of 547 distinct table structures were identified, validating use-case-driven extraction over universal approaches. Consolidation produced machine learning-ready datasets with 16,834 rows by mapping 2,856 column variants to 147 normalized patterns through systematic schema discovery.

Key findings for practitioners: mid-sized architectures (7--8B parameters) with domain-specific fine-tuning should be prioritized over generic large models; prompt complexity must be aligned with accuracy requirements and computational constraints; domain knowledge is necessary for semantic unification beyond syntactic normalization.

Future work should explore hybrid human-machine workflows, active learning for continuous improvement and distributed processing for institutional-scale deployment. This methodology provides a foundation for intelligent document processing in scientific and technical domains.

\section*{Acknowledgment}

This work was funded by PETRONAS Research Sdn. Bhd. under the PETRONAS-ACADEMIA COLLABORATION DIALOGUE 2023 grant. This research was conducted at the Faculty of Computing and Informatics, Universiti Malaysia Sabah. The authors thank laboratory personnel for providing access to technical report archives.
...
\bibitem{tamin2022}
O. Tamin, E. G. Moung, J. A. Dargham, F. Yahya, S. Omatu, and L. Angeline, ``Machine Learning for Plastic Waste Detection: State-of-the-art, Challenges, and Solutions,'' in \textit{Proc. Int. Conf. Communications, Information, Electronic and Energy Systems (CIEES)}, Veliko Tarnovo, Bulgaria, 2022, pp. 1--6, doi: 10.1109/CIEES55704.2022.9990703.

\bibitem{dargham2015}
J. A. Dargham, A. Chekima, E. G. Moung, and S. Omatu, ``The Effect of Training Data Selection on Face Recognition in Surveillance Application,'' in \textit{Distributed Computing and Artificial Intelligence, 12th Int. Conf.}, Advances in Intelligent Systems and Computing, vol. 373. Springer, Cham, 2015, pp. 223--230, doi: 10.1007/978-3-319-19638-1\_26.

\bibitem{dargham2010}
J. A. Dargham, A. Chekima, and E. G. Moung, ``Radon transform for face recognition,'' \textit{Artif. Life Robotics}, vol. 15, pp. 359--362, 2010, doi: 10.1007/s10015-010-0827-2.
